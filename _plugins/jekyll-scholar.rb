  ...
  class Scholar
    module Utilities
      def reference_tag(entry, index = nil)
        ...
        # link doi
        reference = reference.gsub(/\bdoi:(10[.][0-9]{4,}(?:[.][0-9]+)*\/(?:(?![\"&\'<>])\S)+)\b/) { |c| "<a href=\"http://dx.doi.org/#{$1}\" target=\"_blank\">#{$&}</a>" }
        # link urls
        reference = reference.gsub(/\[([^\]]*)\]\(((?:(?:https?|ftp):\/)?\/[-_.!~*\'()a-zA-Z0-9;\/?:\@&=+\$,%#]+)\)/) { |c| "<a class=\"btn btn-primary btn-xs\" href=\"#{$2}\" target=\"_blank\">#{$1}</a>" }
